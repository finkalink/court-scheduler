...
\- \[ ] \*\*`supabase/migrations/0007\_court\_slot\_size.sql` has NOT been applied to the live Supabase project yet\*\* \\-\\- unlike prior migrations, this one couldn't be applied by Claude directly (no DB connection string or `supabase login` access token available in this environment; PostgREST/service\\-role key can do row writes but not DDL). Needs to be run manually via the Supabase SQL editor (or `supabase db push` if the CLI is logged in locally) before the block\\-size picker will actually work end\\-to\\-end \\-\\- until then, `courts.slot_size_minutes` doesn't exist and any insert through `createCourt` will fail.

...
- [ ] Public-facing profiles (follow-up to user profiles, not yet scoped) -- a player-controlled, opt-in page sharing stats (win/loss record, games played, etc.) computed from their event/match history, visible to other players only if the owner chooses to share it. Needs its own sharing/visibility design and stats computation. Flagged in the user-profiles spec's "Future work" section as the natural next step now that profiles themselves exist.

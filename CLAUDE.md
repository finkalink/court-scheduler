...
- [ ] User profiles -- spec approved (`docs/superpowers/specs/2026-09-01-user-profiles-design.md`), not yet planned/implemented. Wires up `users.name` (existed unused since `0001_init.sql`) plus new `gender`/`skill_level` columns, a signup/`/profile` flow, and gates registration for non-`open_play` events on a complete profile. Once shipped, follow-up backlog: **public-facing profiles** -- a player-controlled, opt-in page sharing stats (win/loss record, games played, etc.) computed from their event/match history, visible to other players only if the owner chooses to share it. Needs its own sharing/visibility design and stats computation, not scoped yet -- flagged in the user-profiles spec's "Future work" section as the natural next step once profiles themselves exist.

...
- [ ] Security follow-up from the user-profiles final review: `public.users.email` is writable by its owner via the existing `users update own` RLS policy (all columns, no column-level restriction), and `find_registered_user_by_email` (`supabase/migrations/0021_team_roster_invites.sql`, from the team-roster-invite plan) resolves a captain's teammate-invite lookup against `public.users.email` rather than `auth.users.email` (which `claim_pending_team_invites`, the sibling RPC in the same migration, correctly uses instead). A player could edit their own `public.users.email` to someone else's real address and get linked into a captain's roster in that person's place. Pre-existing since the team-roster-invite plan shipped, only surfaced now because the user-profiles work was the first code to ever write to `public.users` from the app at all, which makes "this table is a user-writable mirror of identity" a load-bearing property going forward. Needs either restricting which columns `users update own` allows (e.g. exclude `email`) or pointing `find_registered_user_by_email` at `auth.users.email` instead.

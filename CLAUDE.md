...
\- \[x] Location admin page to manage sites and times \\-\\- settings (name/address/timezone) were already editable via the "Edit location" disclosure (see the entry below this one). Added the "times" half: weekly hours can now have more than one range per day (e.g. 9:00 AM–12:00 PM and 4:00–9:00 PM \\-\\- up to `MAX\_RANGES\_PER\_DAY` \\= 3, in `src/lib/availability.ts`), both on the per\\-court editor at `/admin/locations/\[locationId]/courts/\[courtId]` and via a new "Set hours for all courts" bulk action on `/admin/locations/\[locationId]` (`applyLocationHours` in `src/app/admin/actions.ts`) that applies one weekly schedule to every court at that location in one save, overwriting each court's existing rules. Also added per\\-date custom hours on the court editor (`saveDateOverride`/`clearDateOverride`) so a court manager can block off part of a specific day (e.g. maintenance) or set different hours for one date, also supporting multiple ranges \\-\\- this uses the existing `slot\_overrides` table with multiple rows per date rather than a schema change. `computeOpenSlots` (`src/lib/availability.ts`) now unions all matching rule/override rows for a day instead of taking just one. No schema/migration needed \\-\\- `availability\_rules` and `slot\_overrides` already allowed multiple rows per court/day and per court/date; only the query logic, save actions, and forms assumed a single row.
...
\- \[ ] Availability reconfiguration \\-\\- rework `availability\_rules` from a single open/close block per day into: an admin sets general open hours for the day, then can individually block or re\\-enable specific 30\\-min slots within that window (rather than the whole window being uniformly open). Partially addressed by the multi\\-range hours work above (an admin can now express "closed" gaps within a day as separate open ranges, both in the weekly template and via a per\\-date override), which covers the common case without a schema change. Still open: a true per\\-30\\-min\\-slot toggle grid \\-\\- today's ranges are still coarse\\-grained (typed open/close times), not click\\-to\\-block individual slots.
...
\- \[ ] Title case for page titles/headings \\-\\- headings like "Find a court", "My bookings", "Confirm booking", "Admin dashboard" are sentence\\-case today; switch to title case ("Find a Court", "My Bookings") across all page `<h1>`/`<h2>`s and the sidebar nav labels for consistency.

\- \[ ] Sidebar visual polish \\-\\- `AppShell`'s sidebar (`src/components/AppShell.tsx`) is a flat opaque white (`bg-white`), which was a deliberate fix for text\\-contrast against the app's dark\\-mode `body` color (see the nav\\-sidebar entry above), but reads as jarring/clashing next to the rest of the app's dark theme. Revisit with a proper dark\\-mode\\-aware background (or a semi\\-transparent/blurred treatment) instead of flat white, and reconsider the fixed `w-64` width at the same time.

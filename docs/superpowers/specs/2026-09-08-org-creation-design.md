...
### Accepted risk: non-atomic two-step insert

A self-serve creation is two separate `INSERT` statements from the server action (organizations, then org_members), not one atomic transaction. If the request dies between them (network failure, server restart), the result is an org row with `is_active = false` and zero members — which the new `org_members` policy's "zero members" check makes claimable by any other authenticated user's *own* self-serve attempt (they'd need to already know or guess that UUID, which is not enumerable through any existing read path, since `organizations select all` returns all orgs but a truly abandoned shell has no name collision risk — worst case, a stranger's next self-serve attempt inserts *their own* ownership row into that same empty, inactive, dataless shell instead of creating a fresh one). Since the shell carries no location/court/booking data and stays invisible to the public regardless of who nominally owns it, this is accepted as a low-severity, low-likelihood edge case rather than justifying a database function/transaction for it. The implementation still includes a best-effort compensating cleanup (delete the org row if the membership insert fails) to shrink the window, but this is a nice-to-have, not a security boundary.

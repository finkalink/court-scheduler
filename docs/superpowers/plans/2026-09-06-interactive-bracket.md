...
- `bracketry`'s bundle is not SSR-safe: it references `document` in an unguarded top-level feature-detection statement, executed the instant its module is evaluated. Next.js still server-renders a `"use client"` component's module graph once for the initial HTML, so a plain `import { createBracket } from "bracketry"` anywhere in that graph crashes with `ReferenceError: document is not defined` on the server — confirmed by reproducing it in plain Node against the actual published bundle. `BracketryTreeView` (and only it, since it's the sole file that imports `bracketry`) must be loaded via `next/dynamic` with `{ ssr: false }` wherever it's rendered, never as a plain static import.
